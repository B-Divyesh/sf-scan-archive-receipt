...
const blankBatch = (): Batch => ({
  ...
  customPattern: '{prefix}-{order}', defaultDate: 'Undated', defaultRights: 'Rights status not evaluated', notes: '', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), items: []
});
...
function render(): void {
  ...
  shell(`<main id="main">
    ...
    <section class="workflow" id="workspace" aria-labelledby="workspace-title"><div class="section-head"><div><p class="eyebrow">ACTIVE RECEIPT</p><h2 id="workspace-title">Batch workbench</h2></div><ol class="stepper" aria-label="Batch progress"><li class="done">1 Describe</li><li class="${batch.items.length?'done':'active'}">2 Import</li><li class="${batch.items.length && complete<batch.items.length?'active':complete?'done':''}">3 Verify</li><li class="${complete && complete===batch.items.length?'active':''}">4 Export</li></ol></div>
      ...
      ${batch.items.length ? `<section class="defaults"><div><h3>03 / Apply batch defaults</h3><p>Fill blanks across the batch. Existing item details stay untouched.</p></div><label>Approximate date<input id="default-date" value="${escapeHtml(batch.defaultDate)}"></label><label>Rights note<input id="default-rights" value="${escapeHtml(batch.defaultRights)}"></label><button id="apply-defaults">Fill blank fields</button></section><section aria-labelledby="scans-title"><div class="list-head"><h3 id="scans-title">Scan order <span>${batch.items.length}</span></h3><p>Use the arrow buttons to match physical order.</p></div><ol class="scan-list">${rows}</ol></section>` : `<section class="empty-state"><span aria-hidden="true">▦</span><h3>No scans on the bench yet</h3><p>Start with one folder or album section. Files are hashed in the order your browser provides them, then you can correct that order.</p></section>`}
...
async function addFiles(files: File[]): Promise<void> {
  ...
  try {
    for (const file of images) {
      ...
      const item: ScanItem = {id:crypto.randomUUID(),order,originalName:file.name,stableName:'',type:file.type||'application/octet-stream',size:file.size,checksum:await sha256(file),sourceItem:'',page:'',approximateDate:batch.defaultDate,rights:batch.defaultRights,notes:'',blob:file};
...
function bindEvents(): void {
  ...
  document.querySelector('#apply-defaults')?.addEventListener('click', () => {const date=(document.querySelector<HTMLInputElement>('#default-date')!.value||'Undated');const rights=(document.querySelector<HTMLInputElement>('#default-rights')!.value||'Rights status not evaluated'); batch!.defaultDate=date;batch!.defaultRights=rights;batch!.items.forEach(item=>{if(!item.approximateDate.trim())item.approximateDate=date;if(!item.rights.trim())item.rights=rights});updateNames();laterSave();render();announce('Blank date and rights fields filled.');});
...
async function start(): Promise<void> {
  ...
  if('serviceWorker' in navigator){const registration=await navigator.serviceWorker.register('/sw.js');if(registration.waiting)showUpdate();registration.addEventListener('updatefound',()=>registration.installing?.addEventListener('statechange',()=>{if(registration.waiting&&navigator.serviceWorker.controller)showUpdate()}));}
}
